...
router.get('/:id/client-milestones', requireAuth(), requireSuperAdmin, async (req, res) => {
  try {
    const project = await Project.findById(req.params.id).populate('clientMilestones');
    if (!project) {
      return res.status(404).json({ error: 'Project not found' });
    }
