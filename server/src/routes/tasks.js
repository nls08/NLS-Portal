...
router.get('/', requireAuth(), async (req, res) => {
  try {
    const tasks = await Task.find().sort({ createdAt: -1 }).populate('assignee').populate('project').populate('milestone')
...
router.get('/:id', requireAuth(), async (req, res) => {
  try {
    const task = await Task.findById(req.params.id).populate('assignee').populate('project').populate('milestone');
    if (!task) {
