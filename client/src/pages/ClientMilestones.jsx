...
const ClientMilestones = () => {
  ...
  const {
    ...
    projects,
    fetchProjects,
    clientMilestoneLoading, setClientMilestoneLoading, clientMilestones, setClientMilestones, fetchClientMilestones,
    profileRole
  } = useApp();
  ...
  console.log(clientMilestones, "clent milestones");
  ...
  const handleCreateMilestone = async (milestoneData) => {
    ...
    try {
      const response = await axios.post("/api/client-milestones", milestoneData, {
        headers: { Authorization: `Bearer ${token}` }
      });
      await fetchClientMilestones();
      setClientMilestones([response.data, ...clientMilestones]);
      toast.success("Client milestone created successfully!");
    } catch (error) {
  ...
  const handleUpdateMilestone = async (milestoneData) => {
    ...
    try {
      const response = await axios.put(
        `/api/client-milestones/${selectedMilestone._id}`,
        milestoneData, {
        headers: { Authorization: `Bearer ${token}` }
      }
      );
      ...
      toast.success("Client milestone updated successfully!");
    } catch (error) {
  ...
  const handleDeleteMilestone = async (milestoneId) => {
    ...
    try {
      await axios.delete(`/api/client-milestones/${milestoneId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      await fetchClientMilestones();
      setClientMilestones(clientMilestones.filter((m) => m._id !== milestoneId));
      toast.success("Client milestone deleted successfully!");
    } catch (error) {
    ...
    <div className="p-6 bg-gray-50 dark:bg-gray-900 min-h-screen">
      <div className="max-w-[100%] mx-auto">
        ...
        >
          <div className="space-y-4">
            <div
              className="prose prose-sm max-w-none mb-4 dark:text-gray-300 border rounded-md p-2"
